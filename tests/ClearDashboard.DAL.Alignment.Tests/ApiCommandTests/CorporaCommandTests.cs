		...
		public async void GetSampleRows()
		{
			try
			{
				...
				foreach (var tokensTextRow in rowsByVerseRange.Rows)
				{
					...
					Assert.All(tokensTextRow.PaddedTokens, t => Assert.NotNull(t.Token.TokenId));
					...
					//display tokenIds
					var tokenIds = string.Join(" ", tokensTextRow.PaddedTokens.Select(t => t.Token.TokenId.ToString()));
					Output.WriteLine($"tokenIds: {tokenIds}");

					//display tokens tokenized
					var tokensText = string.Join(" ", tokensTextRow.PaddedTokens.Select(t => t.Token.SurfaceText));
					Output.WriteLine($"tokensText: {tokensText}");

					//display tokens detokenized
					var detokenizer = new LatinWordDetokenizer();
					var tokensTextDetokenized =
						detokenizer.Detokenize(tokensTextRow.PaddedTokens.Select(t => t.Token.SurfaceText).ToList());
